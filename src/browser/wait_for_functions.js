...
page.waitForSelector("button.onetrust-close-btn-handler", { timeout: 0 }).then(function (elementHandle) {
    logger.info("Dismissing cookie dialog");
    elementHandle === null || elementHandle === void 0 ? void 0 : elementHandle.click();
}).catch(function (error) { return logger.warn("Dismissing cookie dialog failed:", error); });
logger.await("Waiting for user to log in");
var statementsButton = await page.waitForFunction(function () {
...
var selectAnAccountButton = await page.waitForFunction(function () {
...
var accounts = await page.waitForFunction(function (selectAnAccountButton) { return __awaiter(void 0, void 0, void 0, function () {
    var spans;
...
var numAccounts = await accounts.evaluate(function (elementList) { return elementList.length; });
logger.info("Found accounts: ".concat(numAccounts));
var accountNames = [];
for (var i = 0; i < numAccounts; i++) {
    var accountName = await accounts.evaluate(function (elementList, i) { return elementList[i].innerText; }, i);
    logger.info("Account #".concat(i, ": ").concat(accountName));
    accountNames.push(accountName);
}
logger.info("Clicking account: ".concat(accountNames[0]));
accounts.evaluate(function (elementList) { elementList[0].click(); });
